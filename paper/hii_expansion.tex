...
\begin{equation}
  \label{eq:momentum}
  \frac{d(M_\mathrm{sh}\dot{R})}{dt}
    = 4\pi R^2\!\left(P_\mathrm{in} - n(R)\,m_H\,\dot{R}^2\right),
\end{equation}
where the second term on the right is the ram-pressure deceleration from
sweeping up ambient gas.  Expanding the left-hand side and using the mass
equation $\dot{M}_\mathrm{sh} = 4\pi R^2 n(R) m_H v$ (with $v\equiv\dot R$)
yields the classic ODE system:
\begin{align}
...
Neglecting the ram-pressure term and assuming instantaneous ionization
balance, Equation~(\ref{eq:momentum}) reduces to \citep{Spitzer1978}
\begin{equation}
...
\begin{equation}
  \label{eq:spitzer}
  R(t) = R_\mathrm{st}\left[1 + \frac{7}{4}\frac{c_\mathrm{II}}{R_\mathrm{st}}\,t\right]^{4/7}.
\end{equation}
...
residual arises because the quasi-static solution neglects the ram-pressure
term $n m_H v^2$.

\subsection{Physical Origin and Magnitude of the Ram Pressure Term}
\label{sec:ram_pressure}

\paragraph{Origin.}
As the shell sweeps outward at velocity $v$, it engulfs a column of ambient
gas of surface density $n(R)\,m_H\,v\,dt$ per unit area in time $dt$.
Bringing this mass from rest to the shell velocity $v$ requires momentum
$n(R)\,m_H\,v^2\,dt$ per unit area, which must be supplied by the net
pressure force.  Integrated over the shell surface $4\pi R^2$, the ram
pressure force opposing the shell motion is
\begin{equation}
  \label{eq:Fram}
  F_\mathrm{ram} = 4\pi R^2\,n(R)\,m_H\,v^2.
\end{equation}
This term appears explicitly in Equation~(\ref{eq:momentum}) and in the
ODE system (\ref{eq:ode_classic}).

\paragraph{Ram pressure is never negligible: the quasi-static identity.}
The ratio of the ram pressure force to the thermal driving force
$F_\mathrm{th} = 4\pi R^2 P_\mathrm{in}$ is
\begin{equation}
  \label{eq:ram_ratio}
  \frac{F_\mathrm{ram}}{F_\mathrm{th}}
    = \frac{n(R)\,m_H\,v^2}{2\,n_i(R)\,k_B T}.
\end{equation}
Along the Spitzer quasi-static trajectory
$v = c_\mathrm{II}(R_\mathrm{st}/R)^{3/4}$,
with $n_i \propto R^{-3/2}$ for uniform density, both numerator and
denominator scale identically as $R^{-3/2}$, giving
\begin{equation}
  \label{eq:ram_ratio_qstatic}
  \left.\frac{F_\mathrm{ram}}{F_\mathrm{th}}\right|_\mathrm{Spitzer}
    = \frac{n_0\,m_H\,c_\mathrm{II}^2}{2\,n_0\,k_B T}
    = \frac{m_H c_\mathrm{II}^2}{2\,k_B T} = 1
    \quad \text{(at all times)}.
\end{equation}
This result is not a coincidence: the quasi-static approximation is
precisely the condition $P_\mathrm{in} = n\,m_H\,v^2$ (zero net force,
$\dot{v}=0$), which is identical to $F_\mathrm{ram}/F_\mathrm{th}=1$.
\emph{Ram pressure is therefore never negligible; it is always exactly equal
to the thermal driving force along the Spitzer trajectory.}

\paragraph{Why the no-ram ODE diverges from Spitzer.}
Disabling the ram pressure term in the ODE does \emph{not} recover the
Spitzer quasi-static solution (Figure~\ref{fig:ram_pressure}).  In fact,
the no-ram ODE diverges from Spitzer regardless of the initial velocity
$v_0$:
\begin{itemize}
  \item \textbf{D-critical initial condition} ($v_0 = c_\mathrm{II}$):
    the full ODE has zero net force at $t=0$ (quasi-static balance), but
    the no-ram ODE sees an immediate unbalanced force $4\pi R_\mathrm{st}^2
    P_\mathrm{in}(R_\mathrm{st})$, which inflates the trajectory permanently.
  \item \textbf{Static initial condition} ($v_0 = 0$): the force at
    $t=0$ is identical (since $F_\mathrm{ram}=0$ when $v=0$), and the
    initial acceleration $\dot{v}(0) = 3c_\mathrm{II}^2/R_\mathrm{st} =
    3c_\mathrm{II}/T_\mathrm{dyn}$ brings the shell to $v \gg c_\mathrm{II}$
    within $\sim T_\mathrm{dyn}$.  Without a restoring deceleration, the
    shell coasts at this large velocity.
\end{itemize}
In both cases the shell asymptotically coasts at constant velocity
($R\propto t$, log-log slope $\to 1$), because $P_\mathrm{in}\propto
R^{-3/2}$ delivers a \emph{finite} total impulse $\Delta v =
\int(4\pi R^2 P_\mathrm{in}/M_\mathrm{sh})\,dt < \infty$ while no
deceleration mechanism exists.

\paragraph{Origin of the $\sim7\%$ offset.}
The $\sim7\%$ late-time offset between the full ODE (with ram pressure,
$v_0=c_\mathrm{II}$) and Spitzer arises not from ram pressure being large
or small, but from the \emph{inertial term} $M_\mathrm{sh}\,\dot{v}$.
The Spitzer quasi-static derivation sets $\dot{v}=0$ throughout; the full
ODE retains a small but non-zero $\dot{v}$ that accumulates over time to
produce the offset.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{../figures/ram_pressure.pdf}
  \caption{Effect of the ram pressure term and initial condition for
    uniform density ($n_0=100$ cm$^{-3}$).
    \emph{Solid blue}: full ODE with ram pressure, D-critical IC
    ($v_0=c_\mathrm{II}$); \emph{dashed orange}: no-ram ODE, D-critical IC;
    \emph{dash-dot green}: no-ram ODE, static IC ($v_0=0$);
    \emph{dotted gray}: Spitzer quasi-static analytic.
    Both no-ram ODEs coast ($R\propto t$) and diverge from Spitzer because
    ram pressure equals $P_\mathrm{in}$ along the quasi-static trajectory
    and can never be neglected.  The $\sim7\%$ offset of the full ODE from
    Spitzer originates from the retained inertial term $M_\mathrm{sh}\dot{v}$.}
  \label{fig:ram_pressure}
\end{figure}
...
\end{equation}
The momentum equation (Equation~\ref{eq:momentum}) is unchanged: ionized gas
that leaves the shell does so at the shell velocity, so the momentum-flux
...
\begin{figure}[ht]
  ...
  \caption{HII region expansion in a uniform medium ($n_0=100$ cm$^{-3}$).
    Solid: classic thin-shell ODE; dashed: Spitzer (1978) analytic solution;
    dotted: $R\propto t^{4/7}$ reference.}
  \label{fig:constant}
\end{figure}
...
for uniform density over $50\,T_\mathrm{dyn}$.  The top panel shows
$R(t)$: the two solutions are nearly indistinguishable, confirming that the
shell-mass correction has negligible effect on the radius evolution.  The
middle panel shows $v(t)$: again nearly identical.  The bottom panel shows
$M_\mathrm{sh}(t)$: the modified ODE gives a systematically lower shell mass at all
times, as expected from the zero initial shell mass and the continuous
mass transfer to the ionized interior.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{../figures/classic_vs_modified.pdf}
  \caption{Comparison of the classic (solid, blue) and modified (dashed,
    orange) thin-shell ODE solutions for uniform density.
    Top to bottom: shell radius $R(t)$, shell velocity $v(t)$, shell
    mass $M_\mathrm{sh}(t)$.  The radius and velocity agree closely;
    the shell mass in the modified solution is lower throughout.}
