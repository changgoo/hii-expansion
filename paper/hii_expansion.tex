...
\subsection{The Ram Pressure Term: Kinematic Origin and Role in the Quasi-static Solution}
\label{sec:ram_pressure}

\paragraph{Kinematic origin.}
The $n(R)\,m_H\,v^2$ term in the $\dot{v}$ equation is not an independent
external force; it arises from expanding the LHS of
Equation~(\ref{eq:momentum}).  The shell sweeps up ambient gas at rest,
bringing it to velocity $v$; this costs momentum $v\,\dot{M}_\mathrm{sh}
= 4\pi R^2 n\,m_H\,v^2$, which must be supplied by the pressure force and
hence decelerates the shell.  The term is conventionally called ``ram
pressure'' and satisfies
\begin{equation}
  \label{eq:Fram}
  F_\mathrm{ram} \equiv 4\pi R^2\,n(R)\,m_H\,v^2
    = v\,\dot{M}_\mathrm{sh}.
\end{equation}

\paragraph{The quasi-static balance.}
The quasi-static approximation sets $\dot{v}=0$ in the $\dot{v}$ equation,
giving
\begin{equation}
  \label{eq:qs_balance}
  P_\mathrm{in}(R) = n(R)\,m_H\,v^2
  \quad\Longrightarrow\quad
  v = c_\mathrm{II}\!\left(\frac{R_\mathrm{st}}{R}\right)^{3/4},
\end{equation}
which is exactly the Spitzer quasi-static velocity.  The ram pressure term
is therefore \emph{essential} to the quasi-static solution; it is the
balance that determines the expansion velocity.  Numerically, $F_\mathrm{ram}
= P_\mathrm{in}$ (ratio $= 1$) everywhere along the Spitzer trajectory,
confirming that it can never be neglected.

\paragraph{Effect of removing the ram pressure term.}
Zeroing $F_\mathrm{ram}$ in the ODE is equivalent to solving
$d(M_\mathrm{sh}v)/dt = 4\pi R^2(P_\mathrm{in} + F_\mathrm{ram})$, i.e.\
adding \emph{extra} momentum beyond the physical thermal driving.  Without
the restoring kinematic deceleration, the quasi-static balance is broken and
the shell accelerates indefinitely.  Since $P_\mathrm{in}\propto R^{-3/2}$
delivers a finite total impulse, the shell ultimately coasts at constant
velocity ($R\propto t$, slope $\to 1$) for any initial velocity $v_0$
(Figure~\ref{fig:ram_pressure}).

\paragraph{Origin of the $\sim7\%$ offset.}
The $\sim7\%$ late-time offset between the full ODE and the Spitzer
analytic formula (Figure~\ref{fig:ram_pressure}) arises from the
\emph{inertial term} $M_\mathrm{sh}\dot{v}$ retained in the ODE.  The
quasi-static derivation sets $\dot{v}=0$ at every instant; the full ODE
has a small but non-zero $\dot{v}$ whose cumulative effect produces the
offset.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\linewidth]{../figures/ram_pressure.pdf}
  \caption{Role of the ram pressure term for uniform density
    ($n_0=100$ cm$^{-3}$).
    \emph{Solid blue}: full ODE, D-critical IC ($v_0=c_\mathrm{II}$);
    \emph{dashed orange}: no-ram ODE, D-critical IC;
    \emph{dash-dot green}: no-ram ODE, static IC ($v_0=0$);
    \emph{dotted gray}: Spitzer quasi-static analytic.
    Both no-ram ODEs coast ($R\propto t$) because the ram pressure term
    is essential for the quasi-static balance; removing it gives an
    unphysically accelerating shell.  The $\sim7\%$ offset of the full ODE
    from Spitzer originates from the retained inertial term $M_\mathrm{sh}\dot{v}$.}
