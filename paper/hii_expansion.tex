...
\citet{HosokawaInutsuka2006} propose an empirical correction that better
matches numerical results: replace $t_\mathrm{dyn}=R_\mathrm{st}/c_\mathrm{II}$
with
\begin{equation}
  \label{eq:tdyn_HI}
  t_\mathrm{dyn} = \frac{\sqrt{3}}{2}\frac{R_\mathrm{st}}{c_\mathrm{II}},
\end{equation}
which can be obtained from the self-consistent definition
$t_\mathrm{dyn} = R_\mathrm{sp}(0)/\dot{R}_\mathrm{sp}(0)$.
